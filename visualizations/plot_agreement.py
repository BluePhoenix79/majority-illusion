"""Figure 5: self-reported confidence vs. behavioral self-consistency.

Self-consistency is the modal-category share across three identical answer
calls. It is displayed beside inline and post-hoc confidence self-reports;
none of these measures is treated as objective factual correctness.
...
def main():
    ...
    for ax, model_id in zip(axes[0], models):
        ...
        raw = [group[group["ratio"] == ratio]["raw_norm"].mean()
               for ratio in ratios]
        inline_report = [
            group[group["ratio"] == ratio]["inline_norm"].mean()
            for ratio in ratios
        ]
        behavioral = [
        ...
        ax.bar([x - width for x in xs], raw, width, color=RAW_COLOR,
               label="Post-hoc self-report", edgecolor=SURFACE, linewidth=2)
        ax.bar(xs, inline_report, width, color=INLINE_COLOR,
               label="Mean inline self-report",
    ...
    fig.suptitle("Self-reported confidence and behavioral stability", fontsize=12)
